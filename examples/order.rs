...
fn main() {
    ...
    let lexicographical_map: std::collections::BTreeMap<&str, &str> =
        MyConfig[path!(fruits)].into();
    dbg!(lexicographical_map);
    ...
    let indexed_map: indexmap::IndexMap<&str, &str> = MyConfig[path!(fruits)].into();
    dbg!(indexed_map);
}
